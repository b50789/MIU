...
window.addEventListener("DOMContentLoaded", function(){
	...
	function toggleControls(n){
		switch(n){
			case "on":
				myVar('tvForm').style.display = "none";
				myVar('clear').style.display = "inline";
				myVar('displayLink').style.display = "none";
				myVar('addNew').style.display = "inline";
				break;
			case "off":
				myVar('tvForm').style.display = "block";
				myVar('clear').style.display = "inline";
				myVar('displayLink').style.display = "inline";
				myVar('addNew').style.display = "none";
				myVar('items').style.display = "none";
				break;
			default:
				return false;	
		}
	...
	function saveShow(key){
		...
		var item 			={};
			item.date 		= ["Date", myVar('date').value];
			item.showname	= ["Show Name", myVar('showname').value];
			item.r_yes		= ["Recurring ?", recValue];
			item.networks	= ["Network:", myVar('networks').value];
			item.important	= ["How Important?", myVar('important').value];
			item.block_yes	= ["Parental Block Needed?", blockValue];
			item.specialnotes	= ["Notes:", myVar('specialnotes').value];
	...
	function getInfo(){
		...
		myVar('items').style.display = "block";
	...
	function editItem(){
		...
		myVar('date').value = item.date[1];
		myVar('showname').value = item.showname[1];
		var radios0 = document.forms[0].repeat;
		for(var i=0; i<radios0.length; i++) {
		...
		myVar('networks').value = item.networks[1];
		myVar('important').value = item.important[1];
		myVar('block_yes').value = item.block_yes[1];
		myVar('specialnotes').value = item.specialnotes[1];

		//Remove the initial listener from the input 'save show' button.
		saveInfo.removeEventListener("click", saveShow);
		//change Submit button value to Edit Button
		myVar('submit').value = "Edit Show";
		var editSubmit = myVar('submit');
	...
	function validate(e){
		...
		var getShowname = myVar('showname');
